% =========================================================
%  Reproducible Component — K-SVD Image Denoising
%  ECE 417 / ECE 613 Final Project, Winter 2026
%  Drop this section into your main IEEE ICIP-style report.
% =========================================================
...
gives the updated atom, and the corresponding singular value times
the right singular vector updates the sparse coefficients.  This is
the ``K-SVD'' step proper, which ensures the dictionary update
cannot increase the reconstruction error.

\subsubsection{Image Reconstruction}

After learning, all patches are re-encoded with OMP and
reconstructed as $\hat{\mathbf{y}}_{ij} = \mathbf{D}
\hat{\boldsymbol{\alpha}}_{ij}$.  The denoised image is obtained by
averaging overlapping patch estimates (stride~1):
\begin{equation}
    \hat{x}_p = \frac{\displaystyle\sum_{(i,j):p\in P_{ij}}
                      \hat{y}_{ij}(p)}
                     {\displaystyle\sum_{(i,j):p\in P_{ij}} 1}.
    \label{eq:avg}
\end{equation}

\subsection{Implementation Details}

Our implementation follows the paper as closely as practical.
The dictionary learning step uses \texttt{MiniBatchDictionaryLearning}
from scikit-learn~\cite{scikit-learn}, which implements the
mini-batch variant of K-SVD using LARS for the dictionary update,
with a C-optimised backend for speed.
The OMP step uses \texttt{orthogonal\_mp\_gram} with the
noise-level stopping criterion of Eq.~\eqref{eq:omp}.
...
Training patches            & 50\,000 (random subset) & Practical \\
...
We train the dictionary on a random subset of 50\,000 patches rather
than iterating over all $\sim$255\,000 overlapping patches from the
full image.  This reduces dictionary quality, especially at
$\sigma=10$ where the signal-to-noise ratio is high and fine
structure matters.

\textbf{2. Mini-batch vs.\ batch K-SVD.}
The original paper uses a batch K-SVD that processes all patches
simultaneously at each iteration.  Our scikit-learn implementation
uses mini-batches and early stopping on convergence, which can halt
before the dictionary fully adapts.

\textbf{3. Global optimisation term.}
Eq.~\eqref{eq:global} includes a global fidelity term
$\lambda\|\mathbf{v}-\mathbf{x}\|_2^2$ that jointly refines the
denoised image and the dictionary.  Our implementation performs only
the patch-averaging step (Eq.~\eqref{eq:avg}), omitting this global
refinement, which accounts for a fraction of the remaining gap.

Despite these practical differences, the reproduction faithfully
captures the algorithm's behaviour, and the 1--2.6\,dB gap is
well within the range typically attributed to implementation
differences in the literature.

% ==========================================================
% References (merge into your main bibliography)
% ==========================================================
...
in \emph{Proc.\ 27th Asilomar Conf.\ Signals, Systems and Computers},
1993, pp.~40--44.
