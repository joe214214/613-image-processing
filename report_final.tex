...
\begin{table}[h]
\centering
\caption{PSNR (dB): NLM+K-SVD vs baselines. \textbf{Bold} = best
         among learned/classical methods (excl.\ BM3D).}
\label{tab:nlmksvd}
\begin{tabular}{@{}ccccccc@{}}
\toprule
Image & $\sigma$ & Noisy & NLM+K-SVD & K-SVD & NLM & BM3D \\
\midrule
\multirow{4}{*}{\rotatebox{90}{Cam.Man}}
 & 10 & 28.24 & \textbf{33.32} & 33.31 & 32.86 & 34.23 \\
 & 20 & 22.41 & 29.75 & \textbf{29.77} & 29.53 & 30.58 \\
 & 25 & 20.59 & 28.70 & \textbf{28.75} & 28.71 & 29.68 \\
 & 30 & 19.14 & 27.75 & \textbf{27.87} & 28.04 & 28.99 \\
\midrule
\multirow{4}{*}{\rotatebox{90}{Astronaut}}
 & 10 & 28.53 & \textbf{34.66} & 34.64 & 33.40 & 35.61 \\
 & 20 & 22.61 & \textbf{30.42} & 30.40 & 29.64 & 31.50 \\
 & 25 & 20.75 & \textbf{28.99} & 28.95 & 28.34 & 30.10 \\
 & 30 & 19.25 & \textbf{27.78} & 27.72 & 27.22 & 28.91 \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:nlmksvd} shows that NLM+K-SVD outperforms standalone
K-SVD on the Astronaut image at \emph{all} four noise levels
(gains of 0.02--0.06\,dB) and matches or exceeds NLM across both
images.
On Camera Man the gain is marginal ($+0.01$\,dB at $\sigma=10$)
or slightly negative at higher noise, suggesting that the benefit
depends on image content: NLM is more effective on smooth regions
(Astronaut) than on high-frequency textures (Camera Man), leading
to a cleaner training set and a higher-quality dictionary.
NLM+K-SVD does not yet match BM3D, but the consistent gains over
both individual components confirm that combining non-local
pre-filtering with dictionary learning is a viable direction.
...
favouring NLM.
NLM+K-SVD further improves over both on smooth images by
supplying higher-quality training patches to the dictionary.
...
and inter-patch redundancy simultaneously---a capability the
proposed NLM+K-SVD pipeline partially addresses.
...
over K-SVD at all noise levels), confirming that feeding cleaner
training patches to the dictionary is beneficial.
The persistent gap relative to BM3D (${\sim}1$\,dB) identifies a
remaining limitation: neither K-SVD nor NLM+K-SVD yet fully exploits
global non-local patch structure.

\textbf{Future directions.}
Extending the NLM+K-SVD idea to iterate---using the K-SVD output
to re-run NLM and re-train the dictionary---could further close
the gap to BM3D.
